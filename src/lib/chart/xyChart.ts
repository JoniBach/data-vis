...
import {
    isValidSeriesData
} from './xy/utils/validator.js';

// DRY Principle: Tooltip Handling
const handleTooltip = (chartTooltip: d3.Selection<HTMLElement, unknown, null, undefined>, d: any, dataKeys: DataKeys) => {
    try {
...
const moveTooltip = (chartTooltip: d3.Selection<HTMLElement, unknown, null, undefined>, event: MouseEvent) => {
...
const featureRegistry: Record<string, FeatureFunction> = {
    ...
    point: createPoints,
    bar: (params: CreateParams, config: any) => createBarsVariant(config?.variant || 'grouped', params),
};
...
// Scales Creation
const createScales = ({ isBarChart, dateDomainUsed, chartWidth, seriesData, dataKeys, xType }: {
    isBarChart: boolean,
    dateDomainUsed: any[],
    chartWidth: number,
    seriesData: any[],
...
// Chart Setup
const setupChart = (container: HTMLElement, width: number, height: number): d3.Selection<SVGSVGElement, unknown, null, undefined> => {
...
const calculateDomains = ({ syncX, syncY, seriesDataArray, dataKeysArray, featuresArray }: {
    ...
    const mergedDateDomain = syncX ? computeMergedDateDomain(seriesDataArray, dataKeysArray) : undefined;
    const mergedValueDomain = syncY ? computeMergedValueDomain(seriesDataArray, dataKeysArray, featuresArray.map(features => features.find(f => f.feature === 'bar' && !f.hide)?.config?.variant || 'grouped')) : undefined;
    return { mergedDateDomain, mergedValueDomain };
};

// XY Chart Setup
const setupXYChart = (container: HTMLElement, seriesData: any[], width: number, height: number, features: Feature[], dataKeys: DataKeys, dateDomain: any[], valueDomain: any[], isBarChart: boolean, syncX: boolean, syncY: boolean, xType: AxisType, yType: AxisType) => {
...
const createXYChartCore = (container: HTMLElement, seriesDataArray: any[][], width: number, height: number, featuresArray: Feature[][], dataKeysArray: DataKeys[], dateDomain: any[], valueDomain: any[], squash: boolean, syncX: boolean, syncY: boolean, merge: boolean, xType: AxisType, yType: AxisType) => {
    d3.select(container).selectAll("*").remove();
...
export const createSeperateXyCharts = (container: HTMLElement, seriesDataArray: any[][], width = 500, height = 300, featuresArray: Feature[][], dataKeysArray: DataKeys[], squash = false, syncX = false, syncY = false, xType: AxisType = 'number', yType: AxisType = 'number') => {
...
export const createMergedXyCharts = (container: HTMLElement, seriesDataArray: any[][], width = 500, height = 300, featuresArray: Feature[][], dataKeysArray: DataKeys[], squash = false, syncX = false, syncY = false, xType: AxisType = 'number', yType: AxisType = 'number') => {
...
export const createSeriesXYChart = (container: HTMLElement, seriesData: any[], width = 500, height = 300, features: Feature[], dataKeys: DataKeys, dateDomain: any[], valueDomain: any[], syncX = false, syncY = false, xType: AxisType = 'number', yType: AxisType = 'number') => {
...
export const createXyChart = (container: HTMLElement, seriesDataArray: any[][], width = 500, height = 300, featuresArray: Feature[][], dataKeysArray: DataKeys[], merge = false, squash = false, syncX = false, syncY = false, xType: AxisType = 'number', yType: AxisType = 'number') => {
...
const shouldShowFeature = (features: Feature[], featureName: string): boolean => features.some(({ feature, hide }) => feature === featureName && !hide);